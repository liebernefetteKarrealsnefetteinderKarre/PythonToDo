...
class ToDoGameApp:
    def __init__(self):
        ...
        self.tasks = []
        self.task_selection_dropdown = None
    ...
    def start(self):
        ...
        with ui.card():
            ...
            self.task_selection_dropdown = ui.select([], label="Wähle eine Quest zum Abschließen")
    
    def load_data(self):
        connection = sqlite3.connect("database.db")
        cursor = connection.cursor()

        cursor.execute("SELECT name, race, age FROM characters")  # Korrigiert: Daten aus characters-Tabelle
        rows = cursor.fetchall()
        for row in rows:
            name, race, age = row
            character = Character(name, race, age, level=1, xp=0)
    ...
    def update_character_buttons(self):
        ...
        for character in self.characters:
            ui.button(character.name, on_click=lambda c=character: self.select_character(c))
